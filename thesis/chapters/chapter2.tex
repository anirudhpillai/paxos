...
\begin{enumerate}
  \item Whenever an acceptor accepts a value, it should send the accepted proposal
    to all the learners. The learners will then know when a majority of acceptors
...
As you can see from \ref{fig:DiselStateSpace},
A \texttt{statelet} is a component in the protocol and consists of the \texttt{MessageSoup}
...
\texttt{PSentPrep} and \texttt{PWaitPrepResp} are various states the
proposer can be in.
