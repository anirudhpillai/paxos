...
The figure \ref{fig:DiselStateSpace} shows the distributed state space and also the world
components.
...
% The Disel paper \cite{9} defines the global system state as follows.
...
% protocol-specific component, consisting of a `message soup' \texttt{MS} and a per-node
% local state (\texttt{DistLocState}).''

As you can see from \ref{fig:DiselStateSpace}, a protocol $P$ in Disel is defined as a tuple
of the \texttt{Coherence}, the set of send transitions and the set of recieve transitions.
